...
/*
In order for our view to receive keyboard events,
it must be able to accept first responder status.
*/
...
/* 
CPResponder's keyDown: method.

You can find a complete list of all the CPFunctionKey's in
CPEvent_Constants.j File Reference.
*/
- (void) keyDown:(CPEvent)theEvent
{
    var message = @"";
    /* console.info(@"Modifier flags: ", [theEvent modifierFlags]); */

    switch([theEvent modifierFlags]){
        case CPShiftKeyMask:
            message = @"shift + ";
            break;
        case CPCommandKeyMask:
            message = @"cmd + ";
            break;
        case CPControlKeyMask:
            message = @"ctrl + ";
            break;
        case CPAlternateKeyMask:
            message = @"alt + ";
            break;
    }
